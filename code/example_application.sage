"""
    Here, we want to check if a point R is in the subgroup generated by P
    But this is not as easy as on elliptic curves! Often, we may find that <P, R> is completely isotropic
    and so we can't use simple pairing checks.
    Instead, we must generate a support point Q on a non-isotropic part, and differentiate the two cases
    using three pairings and a dlog. This is similar to the MOV attack, in some sense.
"""
